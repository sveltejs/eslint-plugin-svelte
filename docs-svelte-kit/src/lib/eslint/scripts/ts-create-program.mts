...
export async function createVirtualCompilerHost(
	...
	for (const [key, module] of await Promise.all(
		Object.entries({
			...
			// @ts-expect-error -- ignore
			...import.meta.glob('../../../../../node_modules/svelte/types/{runtime,shared}/**/*.d.ts', {
				as: 'raw'
			})
		}).map(async ([key, get]) => [
			key,
	...
	host.compilerHost.resolveModuleNames = function (moduleNames, containingFile) {
	...
	host.compilerHost.getSourceFile = function (fileName, languageVersionOrOptions, ...args) {
		...
		if (this.fileExists(fileName)) {
			return original.getSourceFile.apply(this, [fileName, languageVersionOrOptions, ...args]);
		}
		// Avoid error
		// eslint-disable-next-line no-console -- Demo debug
