  ...
  create(context) {
      ...
      if (openingOption === "always") {
        ...
        if (openingBrace.range[1] !== firstToken.range[0]) {
          context.report({
            ...
            fix: (fixer) =>
              fixer.removeRange([openingBrace.range[1], firstToken.range[0]]),
          })
        }
      }
      if (!closingBrace) {
      ...
      if (
        ...
        if (closingBrace.range[0] !== lastToken.range[1]) {
          context.report({
            ...
            fix: (fixer) =>
              fixer.removeRange([lastToken.range[1], closingBrace.range[0]]),
          })
        }
      }
    }
    ...
    return {
      ...
      SvelteIfBlock(node) {
        ...
        const closeBlockOpeningToken = sourceCode.getTokenBefore(
