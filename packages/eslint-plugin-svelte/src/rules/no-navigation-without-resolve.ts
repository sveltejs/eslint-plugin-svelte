			...
			SvelteAttribute(node) {
				...
				if (
					...
					(node.value[0].type === 'SvelteMustacheTag' &&
						...
						!isResolveCall(context, node.value[0].expression, resolveReferences, tsTools))
				) {
...
function checkGotoCall(
	...
	if (!isResolveCall(context, url, resolveReferences, tsTools)) {
...
function checkShallowNavigationCall(
	...
	const url = call.arguments[0];
	if (!expressionIsEmpty(url) && !isResolveCall(context, url, resolveReferences, tsTools)) {
