...
type ValidTestCases = ReturnType<typeof loadTestCases>['valid'];
type InvalidTestCases = ReturnType<typeof loadTestCases>['invalid'];
...
const loadedCases = loadTestCases('prefer-const');
// This tests cases have been imported/adapted from eslint/prefer-const rule
const ESLINT_RULE_TEST_CASES = {
	valid: [
		'var x = 0;',
		'let x;',
		'let x; { x = 0; } foo(x);',
...
tester.run('prefer-const', rule as any, {
	valid: [...ESLINT_RULE_TEST_CASES.valid, ...loadedCases.valid],
	invalid: [...ESLINT_RULE_TEST_CASES.invalid, ...loadedCases.invalid]
});
